...
def train_one_epoch(
        model, loss_fn, opt, loader, sim_len, local_rank,
        scaler=None, mixup=None, distributed=False
):
    ...
    for img, ys in loader:
        ...
        print("spikes", spikes.shape, "ys", ys.shape)

        loss = loss_fn(spikes, ys)
        loss.backward()
        opt.step()
...
def eval_one_epoch_IF_speed(
        model, test_dataloader, sim_len, trunc_lenth=1000
):
    model.eval()
    starter, ender = torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)
    ...
    with torch.no_grad():
        ...
        return torch.stack([tot_time[i] for i in range(sim_len)
                            if i > 0 and (i & (i + 1)) == 0], dim=0) / trunc_lenth
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--dataset', type=str,
        default='CIFAR100', help='Dataset name'
    )
    parser.add_argument(
        '--datadir', type=str,
        default='/home/cifar100/', help='Directory where the dataset is saved'
    )
    parser.add_argument(
        '--savedir', type=str,
        default='/home/', help='Directory where the model is saved'
    )
    parser.add_argument(
        '--trainsnn_epochs', type=int,
        default=300, help='Training Epochs of SNNs'
    )
    parser.add_argument(
        '--neuron_type', type=str, default='ParaInfNeuron'
    )
    parser.add_argument(
        '--net_arch', type=str, help='Network Architecture'
    )
    parser.add_argument(
        '--batchsize', type=int, default=64, help='Batch size'
    )
    parser.add_argument(
        '--time_step', type=int, default=4, help='Training Time-steps for SNNs'
    )
    ...
    optimizer = SGD(model.parameters(), lr=args.lr, momentum=0.9, weight_decay=args.wd, nesterov=True)
